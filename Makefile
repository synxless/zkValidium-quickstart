...
DOCKERCOMPOSEDEPLOYTRUSTLESS := cdk-validium-deploy-trustless
DOCKERCOMPOSEDAC := cdk-validium-data-availability
DOCKERCOMPOSESETUPDACMOCKL1 := dac-setup-committee-mock-l1
DOCKERCOMPOSESETUPDACTRUSTLESS := dac-setup-committee-trustless
...
RUNDEPLOYTRUSTLESS := $(DOCKERCOMPOSE) up -d $(DOCKERCOMPOSEDEPLOYTRUSTLESS)
...
RUNSETUPDACTRUSTLESS := $(DOCKERCOMPOSE) up -d $(DOCKERCOMPOSESETUPDACTRUSTLESS)
...
.PHONY: run-trustless
run-trustless: ## Runs a full node and deploy contracts to L1 testnet TRUSTLESS
	$(RUNDEPLOYTRUSTLESS)
	$(RUNSETUPDACTRUSTLESS)
	$(MAKE) run-db
	sleep 2
	$(RUNZKPROVER)
	$(RUNAPPROVE)
